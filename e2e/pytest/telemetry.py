...
def validate_initial_telemetry(e2e_cfg, since_after):
    ...
    assert hw_info_cpu.get(
        "architecture", None
    ), "CPU architecture information missing in hardware info telemetry"
    assert hw_info_cpu.get(
        "modelName", None
    ), "CPU model name information missing in hardware info telemetry"
    ...
    assert sysstat_info.get(
        "availMemoryBytes", None
    ), "Available memory information missing in system status telemetry"
    assert sysstat_info.get(
        "bootId", None
    ), "System status boot ID information missing in system status telemetry"
    assert sysstat_info.get(
        "taskCount", None
    ), "System status task count information missing in system status telemetry"
    assert sysstat_info.get(
        "uptimeMillis", None
    ), "System status uptime information missing in system status telemetry"
...
def validate_telemetry_frequency(e2e_cfg):
    ...
    logger.info(
        f"Waiting for {telemetry_period*3} seconds to check that no new telemetry data is received"
    )
