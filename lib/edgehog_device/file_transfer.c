    ...
    char *header_keys[], char *header_values[], char *header_fields[], size_t num_headers)
{
    ...
    for (; idx < num_headers; idx++) {
        ...
        snprintf(
            header_fields[idx], needed, "%s: %s", header_keys[idx], header_values[idx]); // NOLINT
    }
...
exit:
    ...
    k_free(msg->payload.server_to_device.http_header_keys);
    k_free(msg->payload.server_to_device.http_header_values);
...
static void ft_handle_device_to_server(edgehog_device_handle_t edgehog_device, ft_msgq_data_t *msg)
{
    ...
    k_free(msg->payload.device_to_server.http_header_keys);
    k_free(msg->payload.device_to_server.http_header_values);
...
static char **duplicate_string_array(const astarte_data_stringarray_t *src)
{
    ...
    char **dup = k_malloc((src->len + 1) * sizeof(char *));
    if (dup == NULL) {
        return NULL;
    }
