...
{
    EDGEHOG_LOG_DBG("Publishing Edgehog device hardware info");
    astarte_result_t res = astarte_device_set_property(edgehog_device->astarte_device,
        ...
        astarte_data_from_string(CONFIG_ARCH));
    ...
    res = astarte_device_set_property(edgehog_device->astarte_device,
        ...
        astarte_data_from_string(CONFIG_SOC_SERIES));
    if (res != ASTARTE_RESULT_OK) {
    ...
    res = astarte_device_set_property(edgehog_device->astarte_device,
        ...
        astarte_data_from_string(CONFIG_BOARD));
    if (res != ASTARTE_RESULT_OK) {
    ...
    res = astarte_device_set_property(edgehog_device->astarte_device,
        ...
        astarte_data_from_string(CONFIG_SOC_FAMILY));
    if (res != ASTARTE_RESULT_OK) {
    ...
    if (hardware_info_get_memory_size(&memory_size)) {
        res = astarte_device_set_property(edgehog_device->astarte_device,
            ...
            astarte_data_from_longinteger(memory_size));
        if (res != ASTARTE_RESULT_OK) {
