...
ZBUS_SUBSCRIBER_DEFINE(edgehog_ota_internal_subscriber, 1);
ZBUS_CHAN_ADD_OBS(edgehog_ota_chan, edgehog_ota_internal_subscriber, 0);
#endif
...
static void wait_for_reboot(void)
{
    ...
    while (!K_TIMEOUT_EQ(sys_timepoint_timeout(wait_timepoint), K_NO_WAIT)) {
        ...
        if ((zbus_sub_wait(&edgehog_ota_internal_subscriber, &chan, K_MSEC(100)) == 0)
            && (&edgehog_ota_chan == chan) && (zbus_chan_read(chan, &ota, K_NO_WAIT) == 0)
            && (ota.event == EDGEHOG_OTA_REBOOTING_EVENT)) {
            break;
        }
    }
#else
