...
void publish_storage_usage(edgehog_device_handle_t edgehog_device)
{
    ...
    if (res == EDGEHOG_RESULT_OK) {
        astarte_value_pair_t value_pairs[] = { { .endpoint = "totalBytes",
                                                   .value = astarte_value_from_longinteger(
                                                       NVS_PARTITION_SIZE) },
            { .endpoint = "freeBytes", .value = astarte_value_from_longinteger(free_space) } };

        astarte_result_t res = astarte_device_stream_aggregated(edgehog_device->astarte_device,
            io_edgehog_devicemanager_StorageUsage.name, "/" NVS_PARTITION_LABEL, value_pairs,
